...
class Solution:
    def carFleet(self, target: int, position: list[int], speed: list[int]) -> int:
        ...
        pos_speed_pairs = sorted(pos_speed_pairs, key=lambda x: x[0], reverse=True)
        for i in range(len(position)):
            # s = d / t; t = d / s
            time_to_target = (target - pos_speed_pairs[i][0]) / pos_speed_pairs[i][1]
            if len(stack) > 0:
                prev_cars_time = stack[-1]
                if time_to_target > prev_cars_time:
